...
import { toDisplayStatus } from "@/lib/format";
import type {
  AuthSession,
  DoctorQueueItem,
  Medicine,
...
export function DoctorDashboard({
  ...
  useEffect(() => {
    ...
    const loadDoctorQueue =async () => {
      ...
      try {
        const payload = await getDoctorQueue(session.token);
        ...
        setError(extractErrorMessage(error, "Couldn't load today's doctor queue."));
      } finally {
       if (!cancelled) {
          setLoading(false);
        }
  ...
  const handlePrescriptionSubmit= async (event: FormEvent<HTMLFormElement>) => {
   ...
   setPrescriptionSubmitting(true);
    ...
    try {
      await addPrescription(session.token, appointmentId, {
        medicines,
      ...
      setError(extractErrorMessage(error, "Unable to save prescription."));
    } finally {
  ...
  const handleReportSubmit= async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await addReport(session.token, appointmentId, {
     ...
     setError(extractErrorMessage(error, "Unable to save report."));
    } finally {
