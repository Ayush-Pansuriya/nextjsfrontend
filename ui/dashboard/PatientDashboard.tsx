...
import { isAuthError, extractErrorMessage } from "@/api/api";
import {
...
import { formatDate, getTodayInputValue } from "@/lib/format";
import type {
  Appointment,
  AppointmentDetail,
...
export function PatientDashboard({
     ...
     setLoading(true);
      ...
      try {
        const [appointmentsPayload, prescriptionsPayload, reportsPayload] =
          await Promise.all([
            getMyAppointments(session.token),
            getMyPrescriptions(session.token),
            getMyReports(session.token),
         ]);
        ...
        setError(extractErrorMessage(error, "Couldn't load your patient data."));
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  const handleBookAppointment =async (event: FormEvent<HTMLFormElement>) => {
   ...
   setBookingLoading(true);
    ...
    try {
      await scheduleAppointment(session.token, formValues);
      ...
      setError(extractErrorMessage(error, "Couldn't book your appointment."));
    } finally {
  ...
  const handleLoadDetail= async (appointmentId: number) => {
    ...
    try {
      const detail = await getAppointmentDetails(session.token, appointmentId);
     setAppointmentDetail(detail);
      ...
      setError(extractErrorMessage(error, "Couldn't load appointment details."));
    } finally {
     setBusyDtl(false);
    }
  };
