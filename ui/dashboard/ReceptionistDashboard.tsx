...
import { QUEUE_ACTION_LABEL } from "@/lib/constants";
import { getTodayInputValue, queueActions } from "@/lib/format";
import { extractErrorMessage, isAuthError } from "@/api/api";
import { getQueueForDate, changeQueueStatus } from "@/api/clinic-service";
import type { AuthSession, QueueEntry, QueueTransitionStatus } from "@/lib/types";
...
export function ReceptionistDashboard({
  ...
  useEffect(() => {
    ...
    const loadQueue= async () => {
     ...
     try {
        const payload = await getQueueForDate(session.token, queueDate);
       ...
       setError(extractErrorMessage(error, "Unable to load queue entries."));
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  const handleQueueUpdate =async (
    ...
    try {
      await changeQueueStatus(session.token, queueId, nextStatus);
      ...
      setError(extractErrorMessage(error, "Unable to update queue entry."));
    } finally {
      setTempVal(null);
   }
  };
