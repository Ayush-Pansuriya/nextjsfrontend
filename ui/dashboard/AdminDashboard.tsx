...
export function AdminDashboard({
    ...
    const loadDashboard =async () => {
      ...
      try {
       const [clinicPayload, usersPayload] = await Promise.all([
          getClinicSummary(session.token),
          listClinicUsers(session.token),
        ]);
       ...
       setClinicInfo(clinicPayload);
        ...
        setError(extractErrorMessage(error, "Couldn't load admin dashboard data."));
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  const handleSubmit= async (event: FormEvent<HTMLFormElement>) => {
   ...
   const phone =formValues.phone.trim();
    ...
    try {
      await addClinicUser(session.token, {
        name,
        email,
        password,
      ...
      setError(extractErrorMessage(error, "Couldn't create user account."));
    } finally {
      setSubmitting(false);
    }
  };
